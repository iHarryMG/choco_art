...
<?
    ...
    {		
			...
			$testServer = "203.238.36.58";
			$realServer1 = "203.238.36.173";
			$realServer2 = "203.238.36.178";
			         
			//if(($remoteAddr == $testServer) || ($remoteAddr == $realServer1) || ($remoteAddr == $realServer2)){

				include("../../dbconn/common.php");
				$sql="insert into completed_creditcard_order_extra_info ";
				$sql.="(site_cd, tno, order_no, tx_cd, tx_tm, result, lpgm_name, ipgm_mnyx, totl_mnyx, ipgm_time, bank_code, account, noti_id, op_cd, remitter, cash_a_no, result_date)";
				$sql.=" values('$site_cd', '$tno', '$order_no', '$tx_cd', '$tx_tm', '$result', '$lpgm_name', '$ipgm_mnyx', '$totl_mnyx', '$ipgm_time', '$bank_code', '$account', '$noti_id', '$op_cd', '$remitter', '$cash_a_no', NOW())";
				$result = mysql_query($sql, $connect);

				$sql="update shop_order";
				$sql.="set order_paid_amount='$ipgm_mnyx', order_paid_date=NOW(), order_status='결제완료'";
				$sql.="where order_code='$order_no'";
				$result2 = mysql_query($sql, $connect);

				if(!$result || !$result2){
					$success = "222";
					echo "<script>						
							alert('가상계좌 입금 통보정보를 저장하지 못했습니다.');
						  </script>";
				}		
				else{
					$success = "0000";
...
<html>
	...
	<body>
		<form>
			<input type="hidden" name="result" value="<?echo $success?>">
		</form>
	</body>
</html>