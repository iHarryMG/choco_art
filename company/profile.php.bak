...
<HEAD>
  ...
  <LINK href="design/profile_design.css" type=text/css rel=stylesheet>
...
?>
				<ul>
					<li><a href="../membership/logout.php?win=company/profile">로그아웃</a></li>
					<li>|</li>
...
?>					
			<ul>
				<li><a href="javascript:login('company/profile')">로그인</a></li>
				<li>|</li>
...
?>
	...
	<div class="content">
		<img id="title_background" src="../images/title_background/brownrose.jpg">
		<img id="title" src="../images/title_background/profile.png">
		<h5>HOME > ABOUT US > Profile</h5>
	
	<img id="profile" src="../images/menu_images/profile.jpg" height="130" width="400">
	<table id="bigtable" cellpadding=20>
	<tr>
		<td>
		<table id="toptable" cellpadding=10>
			<tr><td colspan=2 id="title"><strong>● 학력</strong></td></tr>
		  <?
			include "../dbconn/common.php";

			$sql="select date, content from academiccareer";
			$result=mysql_query($sql,$connect);
			$fields=mysql_num_fields($result);
			while($row=mysql_fetch_row($result)){
				echo("<tr >");
				echo("<td id='left' width=150px>$row[0]</td>");
				echo("<td id='right'> $row[1] </td>");
				echo("</tr>");
			}
		  ?>

		</table>		
		<table id="middletable" cellpadding=10>
		  <tr><td colspan=2 id="title"><strong>● 주요경력 및 자격</strong></td></tr>
		  <?
			$sql="select date, content from career";
			$result=mysql_query($sql,$connect);
			$fields=mysql_num_fields($result);
			while($row=mysql_fetch_row($result)){
				echo("<tr >");
		...
		<table id="bottomtable" cellpadding=10>
		  <tr><td colspan=2 id="title"><strong>● 방송출연 및 촬영</strong></td></tr>
		  <?
			$sql="select date, content from broadcast";
			$result=mysql_query($sql,$connect);
			$fields=mysql_num_fields($result);
			while($row=mysql_fetch_row($result)){
				echo("<tr >");
	...
	<div id="sidemenu">
		...
		<div class="submenu">
			<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,0,0" width="160" height="200" id="chocoart_sidemenu_aboutus_profile" align="middle">
			<param name="allowScriptAccess" value="sameDomain" />
			<param name="movie" value="design/chocoart_sidemenu_aboutus_profile.swf" />
			<param name="quality" value="high" />
			<param name="wmode" value="transparent" />
			<param name="bgcolor" value="#663300" />
			<embed src="design/chocoart_sidemenu_aboutus_profile.swf" quality="high" wmode="transparent" bgcolor="#663300" width="160" height="200" name="chocoart_sidemenu_aboutus_profile" align="middle" allowScriptAccess="sameDomain" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />
			</object>
		</div><!--.submenu end-->
