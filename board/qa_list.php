...
<HEAD>
  ...
  <LINK href="design/qa_list_design.css" type=text/css rel=stylesheet>
  <script language="javascript">
	function openPopUpWin(recordID){
		window.open("qa_content.php?id="+recordID,"","scrollbars=yes, resizable=no, width=717, height=400");
	}
  </script>
 </HEAD>
...
<? 
			...
			if((1 < $presentPageNumber) && ($presentPageNumber < $totalPageNumber)){ // MiddlePages
				...
				$anotherPageSql = "SELECT qa_id, title, registrant, date, readnum  
				...
				for($i=$startIndex ; $i < ($next + $pageMaxRowNumber); $i++){ // records
					...
					if(strlen($title) > 35){
						...
						echo ("	<td align=center>$recordNumber</td> 
								<td id='title' align=left><a href='#' onclick='openPopUpWin($qa_id)'>$short_title... $answer</a></td> 
						...
						echo ("	<td align=center>$recordNumber</td> 
								<td id='title' align=left><a href='#' onclick='openPopUpWin($qa_id)'>$title $answer</a></td> 
								<td align=center>$registrant</td> 
								<td align=center>$date</td> 
								<td align=center>$readnum</td> ");
					}
...
?>
						<td id='title' align=left><a href='#' onclick='openPopUpWin(<?echo $row['qa_id'];?>)'><?echo $short_title?>...<?echo " ".$answer?></a></td> 
<?
					}
					else{
?>
						<td id='title' align=left><a href='#' onclick='openPopUpWin(<?echo $row['qa_id']?>)'><?echo $row['title']?><?echo " ".$answer?></a></td>
<?
					}
?>
...
?>					
				<td align=center id='write_btn'>
						<a href='qa_write.php?login=<?echo $_SESSION['user_id']?>' target='qa_content'><img src='img/write_btn_darkgreen.png' /></a>
				</td>
			</tr>
		</table>
 </center>
