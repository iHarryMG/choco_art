...
<HEAD>
  ...
  <LINK href="design/freeboard_list_design.css" type=text/css rel=stylesheet>
  <script language="javascript">
	function openPopUpWin(recordID){
		window.open("freeboard_content.php?id="+recordID,"","scrollbars=yes, resizable=no, width=717, height=500");
	}
  </script>
 </HEAD>
 <BODY>
...
<? 
			...
			if((1 < $presentPageNumber) && ($presentPageNumber < $totalPageNumber)){ // MiddlePages
				...
				$anotherPageSql = "SELECT freeboard_id, title, registrant, date, readnum  
									FROM freeboard ORDER BY freeboard_id desc";
				...
				for($i=$startIndex ; $i < ($next + $pageMaxRowNumber); $i++){ // records
					$freeboard_id = mysql_result($anotherPageResult, $i, 0);
					...
					if(strlen($title) > 35){
						...
						echo ("	<td align=center>$recordNumber</td> 
								<td id='title' align=left><a href='#' onclick='openPopUpWin($freeboard_id)'>$short_title...</a></td> 
						...
						echo ("	<td align=center>$recordNumber</td> 
								<td id='title' align=left><a href='#' onclick='openPopUpWin($freeboard_id)'>$title</a></td> 
								<td align=center>$registrant</td> 
								<td align=center>$date</td> 
								<td align=center>$readnum</td> ");
					}
				...
				$nextRecord = 0;
				$sql = "SELECT * FROM freeboard ORDER BY freeboard_id desc";
				...
				if($records < 1){
					...
					$firstPageSql = "SELECT * FROM freeboard ORDER BY freeboard_id desc";
...
?>
						<td id='title' align=left><a href='#' onclick='openPopUpWin(<?echo $row['freeboard_id'];?>)'><?echo $short_title?>...</a></td> 
<?
					}
					else{
?>
						<td id='title' align=left><a href='#' onclick='openPopUpWin(<?echo $row['freeboard_id']?>)'><?echo $row['title']?></a></td>
<?
					}
?>
...
?>					
				<td align=center id='write_btn'>
						<a href='freeboard_write.php?login=<?echo $_SESSION['user_id']?>' target='freeboard_content'><img src='img/write_btn_darkgreen.png' /></a>
				</td>
			</tr>
		</table>
 </center>
