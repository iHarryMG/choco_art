...
<? 
			...
			if((1 < $presentPageNumber) && ($presentPageNumber < $totalPageNumber)){ // MiddlePages
				...
				$anotherPageSql = "SELECT qa_id, title, registrant, date, readnum, registrant_id
									FROM qa ORDER BY qa_id desc";
				...
				for($i=$startIndex ; $i < ($next + $pageMaxRowNumber); $i++){ // records
					$qa_id = mysql_result($anotherPageResult, $i, 0);
					...
					$answer_sql = "SELECT * FROM qa_answer where qa_id='".$qa_id."'";
					...
					if(strlen($title) > 35){
						...
						echo ("	<td align=center>$recordNumber</td> 
								<td id='title'><a href='$PHP_SELF?id=$qa_id'>$short_title... $answer</a></td> 
								<td align=center>$registrant($registrant_id)</td> 
								<td align=center>$date</td>
								<td align=center>$readnum</td> ");
						echo("<td id='btn'><a href='$PHP_SELF?id=$qa_id'>수정</a></td>");
						echo("<td id='btn'><a href='delete_qa_list.php?id=$qa_id'>삭제</a></td>");
					}
					else{
						echo ("	<td align=center>$recordNumber</td> 
								<td id='title'><a href='$PHP_SELF?id=$qa_id'>$title $answer</a></td> 
								<td align=center>$registrant($registrant_id)</td>  
								<td align=center>$date</td> 
								<td align=center>$readnum</td> ");
						echo("<td id='btn'><a href='$PHP_SELF?id=$qa_id'>수정</a></td>");
				...
				$nextRecord = 0;
				$sql = "SELECT * FROM qa ORDER BY qa_id desc";
				...
				if($records < 1){
					...
					$firstPageSql = "SELECT * FROM qa ORDER BY qa_id desc";
...
<?
					$answer_sql = "SELECT * FROM qa_answer where qa_id='".$row['qa_id']."'";
					$answer_result = mysql_query($answer_sql, $connect);
					$answer_records = mysql_num_rows($answer_result);  
					$answer = "";
					if($answer_records > 0){
...
?>
						<td id='title'><a href='<?echo $PHP_SELF?>?id=<?echo $row['qa_id']?>'><?echo $short_title?>...<?echo " ".$answer?></a></td> 
<?
					}
					else{
?>
						<td id='title'><a href='<?echo $PHP_SELF?>?id=<?echo $row['qa_id']?>'><?echo $row['title']?><?echo " ".$answer?></a></td>
<?
					}
?>
							...
							<td id='btn'><a href='<?echo $PHP_SELF?>?id=<?echo $row['qa_id']?>'>수정</a></td>
							<td id='btn'><a href='delete_qa_list.php?id=<?echo $row['qa_id']?>'>삭제</a></td>
						</tr>
<?						
						$recordNumber--;
						$nextRecord += 1;
...
<?
			$record = "";
			$par_id = $_REQUEST['id'];
			$sql = "SELECT * FROM qa where qa_id='$par_id'";
			$result = mysql_query($sql, $connect);
			$record = mysql_fetch_array($result);  	
?>
			<form name="admin_form" action="save_qa_list.php?id=<?echo $par_id?>" method="POST">
...
<?
			$record = "";
			$sql = "SELECT * FROM qa_answer where qa_id='$par_id'";
			$result = mysql_query($sql, $connect);
			$record = mysql_fetch_array($result);  	
			mysql_close($connect);
?>
