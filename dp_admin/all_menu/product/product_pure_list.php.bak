...
<? 
			...
			if((1 < $presentPageNumber) && ($presentPageNumber < $totalPageNumber)){ // MiddlePages
				...
				$anotherPageSql = "SELECT num, code, name, price, state, category FROM product where category='pure' ORDER BY num desc";
				...
				for($i=$startIndex ; $i < ($next + $pageMaxRowNumber); $i++){ // records
					...
					echo("<tr>");
							echo ("<td align=center>$recordNumber</td> 
								   ...
								   <td id='title'><a href='save_product_form.php?win=product_pure_list&id=info&code=$code'>$name</a></td> 
							...
							echo ("<td id='btn'><a href='insert_product_image.php?win=product_pure_list&name=$name&code=$code'>사진</a></td>");
							echo ("<td id='btn'><a href='save_product_form.php?win=product_pure_list&id=edit&code=$code'>수정</a></td>");
							echo ("<td id='btn'><a href='delete_product_list.php?win=product_pure_list&code=$code'>삭제</a></td>");							
					echo("</tr>");
					$recordNumber--;
					$nextRecord += 1;
				}
				...
				$nextRecord = 0;
				$sql = "SELECT * FROM product where category='pure' ORDER BY num desc";
				...
				if($records < 1){
					...
					$firstPageSql = "SELECT num, code, name, price, state, category FROM product where category='pure' ORDER BY num desc";
...
?>
	...
	<div id="btn_register">
		<input id="btn_register" type="button" value="등록" onclick="javascript:window.location.href='save_product_form.php?win=product_pure_list&id=new&code='" style="cursor:hand">
	</div>
 </br>
 </BODY>
</HTML>
