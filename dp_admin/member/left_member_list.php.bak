	...
	<form name="admin_form" action="<?echo $_PHPSELF?>" method="POST">
		...
		<table cellpadding=5> 
			...
			if((1 < $presentPageNumber) && ($presentPageNumber < $totalPageNumber)){ // MiddlePages
				...
				$anotherPageSql = "SELECT *	FROM member where class='$member_class' ORDER BY num desc ";
				...
				for($i=$startIndex ; $i < ($next + $pageMaxRowNumber); $i++){ // records
					...
					echo("<tr>");
						echo ("	<td align=center>$recordNumber</td> 
								<td align=center><a href='left_view_member_info.php?num=$num' target='content'>$name($id)</a></td> 
								<td align=center>$class</td>
								<td align=center>$cell</td>								
								<td align=center>$date</td>

								<td><a href='left_edit_member_info.php?num=$num'>수정</a></td>
								<td><a href='left_delete_member.php?num=$num'>삭제</a></td>");
					echo("</tr>");
					$recordNumber--;
					$nextRecord += 1;
				}
				...
				$nextRecord = 0;
				$sql = "SELECT * FROM member where class='$member_class' ORDER BY num desc";
				...
				if($records < 1){
					...
					$firstPageSql = "SELECT * FROM member where class='$member_class' ORDER BY num desc";
					...
					for($i=0 ; $i < $firstPageRecords; $i++){ // records
						...
						echo ("<tr> 
								...
								<td align=center><a href='left_view_member_info.php?num=$row[num]' target='content'>$row[name]($row[id])</a></td> 
								<td align=center>$row[class]</td> 
								<td align=center>$row[cell]</td> 								
								<td align=center>$row[date]</td>

								<td><a href='left_edit_member_info.php?num=$row[num]'>수정</a></td>
								<td><a href='left_delete_member.php?num=$row[num]'>삭제</a></td>
							  </tr> ");
						$recordNumber--;
						$nextRecord += 1;
						if(9 < $nextRecord){
